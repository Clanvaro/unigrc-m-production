...
export function ProcessOwnerForm({ processOwner, onSuccess, onCancel }: ProcessOwnerFormProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "No se pudo crear el dueño de proceso.",
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "No se pudo actualizar el dueño de proceso.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* NUEVO: Campo Empresa con selector HTML nativo (anti-cache fix) */}
        <FormField
          control={form.control}
          name="company"
          render={({ field }) => (
